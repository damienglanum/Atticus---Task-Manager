...
per size rather than downscaled from one master**. The icon has the same problem
the in-app mark does — eight contours are a smudge below about 56 px — so each
size in the `.icns` is rendered at the ring count that survives it, and
`iconutil` assembles them. Compared side by side, a naive downscale of the 1024
is illegible at 32 px and nearly invisible at 16; the per-size renders hold.
...
The artwork is the contour mark in `#dedede` on the splash's `#0b0c0d`, inside a
rounded square with the 10% margin macOS expects to be baked into the image.
...
papercut. It carries the dark square rather than being bare linework, because a
